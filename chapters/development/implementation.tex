...
However, it is important to note that the \texttt{Sequence} node is a little bit different from the non-reactive sequence presented in Chapter \ref{ch:background}, since when one of its children returns a failure, instead of ticking it again the next time the sequence is ticked, the whole sequence is restarted. The library also provides a sequence node with memory as presented in Chapter \ref{ch:background}, but it was not used in the implementation of the tree. 

In addition, another type of control flow node is used in the tree implementation, which is the decorator node, however, the reason why this node was used in the tree implementation will not be explained in this subsubsection, but in Subsubsection \ref{subsubsec:decator_nodes_impl}.
